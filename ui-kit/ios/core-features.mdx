...
To enable rich text formatting, set `enableRichTextFormatting = true`. To show the formatting toolbar, set `showRichTextFormattingOptions = true`. See the [CompactMessageComposer Rich Text Formatting](/ui-kit/ios/compact-message-composer#rich-text-formatting) section for full configuration details.
