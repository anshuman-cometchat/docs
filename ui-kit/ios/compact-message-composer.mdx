...
| setEnableRichTextFormatting | Master switch to enable or disable rich text formatting (bold, italic, etc.). | `enableRichTextFormatting = true` |
| setRichTextFormattingOptionsVisibility | Controls the visibility of the rich text formatting toolbar. | `showRichTextFormattingOptions = true` |
...
- `enableRichTextFormatting = true` — Master switch that activates formatting. Without this, even manually typed markdown syntax like `**text**` will be sent as plain text.
- `showRichTextFormattingOptions = true` — Shows the formatting toolbar below the input field.

If `enableRichTextFormatting` is set to `false` (the default), all formatting is disabled. Markdown syntax typed in the composer will be sent as plain text, and the toolbar will not appear regardless of other settings.
...
| true | true | Toolbar visible below input, full formatting support |
