...
// With common props
<CometChatCallLogs
  onItemPress={(call: any) => console.log("Selected:", call)}
  onError={(error: CometChat.CometChatException) => console.log("Error:", error)}
  showBackButton={true}
  onLoad={(list: any[]) => console.log("Loaded:", list.length)}
/>
...
<Note>
**Available via:** [UI Kits](/ui-kit/react-native/overview) | [SDK](/sdk/react-native/call-logs) | [REST API](/rest-api/list-calls)
</Note>
...
//code
const onItemLongPressHandler = (prop: { call: any }) => {
  //handle long press
};
...
This action does not change the behavior of the component but rather listens for any errors that occur in the Call Logs component.
...
import { CometChatCallLogs } from "@cometchat/chat-uikit-react-native";
//code
const onLoadHandler = (list: any[]) => {
  console.log("Call logs loaded:", list.length);
};

return <CometChatCallLogs onLoad={onLoadHandler} />;
```
...
return <CometChatCallLogs onEmpty={onEmptyHandler} />;
```
...
| **showBackButton**            | Used to show/hide the back button                    | `showBackButton?: boolean`                                    |
...
| **hideHeader**                | Used to toggle visibility of header                  | `hideHeader?: boolean`                                        |
| **hideLoadingState**          | Used to toggle visibility of Loading state           | `hideLoadingState?: boolean`                                  |
...
You can set Custom `AppBarOptions` to the `CometChatCallLogs` component.
