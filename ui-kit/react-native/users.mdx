...
// Common pattern: Navigate to Messages on user tap
<CometChatUsers
  onItemPress={(user: CometChat.User) => {
    // Navigate to CometChatMessages with user prop
    // navigation.navigate('Messages', { user });
    console.log("Selected user:", user.getUid(), user.getName());
  }}
...
This action does not change the behavior of the component but rather listens for any errors that occur in the Users component.
