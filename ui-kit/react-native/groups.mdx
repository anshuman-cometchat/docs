...
const onPressHandler = (group: CometChat.Group) => {
  //code
};
...
const onLongPressHandler = (group: CometChat.Group) => {
  //code
};
...
const onSelectionHandler = (list: Array<CometChat.Group>) => {
  //code
};
...
This action does not change the behavior of the component but rather listens for any errors that occur in the Groups component.
...
const onLoadHandler = (list: CometChat.GroupMember[]) => {
  console.log("Groups loaded:", list.length);
};

return <CometChatGroups onLoad={onLoadHandler} />;
```
...
return <CometChatGroups onEmpty={onEmptyHandler} />;
```
...
| **selectionMode**         | This method determines the selection mode for groups, enabling users to select either a single group or multiple groups at once. | `selectionMode?: SelectionMode` |
| **hideSubmitButton**      | Used to toggle the visibility of the submit button when selectionMode is enabled.                                               | `hideSubmitButton?: boolean`  |
| **searchPlaceholderText** | Used to set search placeholder text                                                                                             | `searchPlaceholderText?: string` |
| **hideLoadingState**      | Hide the loading skeleton                                                                                                       | `hideLoadingState?: boolean`  |
| **groupTypeVisibility**   | Toggle visibility of group type (public/private/password)                                                                       | `groupTypeVisibility?: boolean` |
| **searchKeyword**         | Set initial search keyword                                                                                                      | `searchKeyword?: string`      |
...
This method sets a predefined list of actions that users can perform when they long press a group in the list. These options typically include:
