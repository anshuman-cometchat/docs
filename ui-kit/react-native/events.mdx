...
CometChatUIEventHandler.addMessageListener("LISTENER_ID", {
  ccMessageSent: ({ message }) => console.log("Sent:", message),
  ccMessageEdited: ({ message }) => console.log("Edited:", message),
  ccMessageDeleted: ({ message }) => console.log("Deleted:", message),
});

// Add conversation listener
CometChatUIEventHandler.addConversationListener("LISTENER_ID", {
  ccConversationDeleted: (conversation) => console.log("Deleted:", conversation),
});

// Remove listeners
CometChatUIEventHandler.removeMessageListener("LISTENER_ID");
CometChatUIEventHandler.removeConversationListener("LISTENER_ID");
```

- **Methods** → [CometChatUIKit Methods](/ui-kit/react-native/methods)
- **Components Overview** → [All Components](/ui-kit/react-native/components-overview)
</Info>
