...
description: "Migrate from CometChat React Native UI Kit v4 to v5 with updated components, simplified theming, and modular architecture."
---
...
- Composite components (e.g., `CometChatConversationsWithMessages`) replaced with modular components (`CometChatConversations`, `CometChatMessageHeader`, `CometChatMessageList`, `CometChatMessageComposer`)
- Theming: `new CometChatTheme({})` with setter methods → `CometChatThemeProvider` with a plain theme object
- Styling: Nested style objects (e.g., `avatarStyle`, `listItemStyle`) → single `style` prop with `DeepPartial<>` types
- Custom views: `ListItemView` → `ItemView`, `TailView` → `TrailingView`, `EmptyStateView` → `EmptyView`
- Full property changes → [Property Changes](/ui-kit/react-native/property-changes)
</Info>
...
<CardGroup cols={2}>
  <Card title="Property Changes" icon="list-check" href="/ui-kit/react-native/property-changes">
