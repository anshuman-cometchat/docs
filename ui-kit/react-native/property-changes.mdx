...
Common v4 → v5 property renames:
- `ListItemView` → `ItemView`
- `TailView` → `TrailingView`
- `EmptyStateView` → `EmptyView`
- `ErrorStateView` → `ErrorView`
- `LoadingStateView` → `LoadingView`
- `disableUsersPresence` → `usersStatusVisibility`
- `[component]Style` → `style` (e.g., `conversationsStyle` → `style`)
- `AppBarOption` → `AppBarOptions`
- `hideSubmitIcon` → `hideSubmitButton`

All style props now use `DeepPartial<>` types for flexible overrides.
</Info>
...
<CardGroup cols={2}>
  <Card title="Upgrading From V4" icon="arrow-up" href="/ui-kit/react-native/upgrading-from-v4">
    Overview of architectural changes between v4 and v5
  </Card>
  <Card title="Component Styling" icon="paintbrush" href="/ui-kit/react-native/component-styling">
    Learn the v5 approach to styling individual components
  </Card>
