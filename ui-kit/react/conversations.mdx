...
<Accordion title="Agent-Parseable Spec Card" defaultOpen>
...
| Real-time events | `CometChatConversationEvents.ccConversationDeleted` |

</Accordion>

## Base Code and Stylesheet

Requires `CometChatUIKit.init()` + `CometChatUIKit.login()` before rendering. Renders a scrollable list of recent conversations.
...
| `title` | `string` | `"Chats"` | Sets the header title text | — |
...
No predefined navigation or routing behavior. All interaction responses are delegated to the callbacks below.
