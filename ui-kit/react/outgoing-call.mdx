...
Show the outgoing call screen after initiating a call.

## When to use this

- You need to initiate voice or video calls.
- You want a default outgoing call UI.
- You need to handle call cancellation.
...
- You must create a `CometChat.Call` via `CometChat.initiateCall()`.
...
- `call` prop is required.
- Use `onCallCanceled` for cancel handling.
...
- **Minimal JSX:** `<CometChatOutgoingCall call={callObject} />` — requires a `CometChat.Call` object from `CometChat.initiateCall()`
...
| --- | --- | --- | --- |
| Render outgoing call | `CometChatOutgoingCall` | `call` | `call={call}` |
| Handle cancel | `CometChatOutgoingCall` | `onCallCanceled` | `onCallCanceled={() => ...}` |
| Disable sounds | `CometChatOutgoingCall` | `disableSoundForCalls` | `disableSoundForCalls={true}` |

## Common pitfalls and fixes

| Symptom | Cause | Fix |
| --- | --- | --- |
| Outgoing UI not shown | Call not initiated | Call `CometChat.initiateCall()` and pass result |
...
**Do I need to initiate the call?**
Yes. `call` must come from `CometChat.initiateCall()`.

**How do I cancel a call?**
Use the `onCallCanceled` callback.
