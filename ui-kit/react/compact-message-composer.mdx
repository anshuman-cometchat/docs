...
**Minimal JSX:** `<CometChatCompactMessageComposer user={chatUser} />`
**Required setup:** `CometChatUIKit.init()` + `CometChatUIKit.login()`
**Key props:** `user` (CometChat.User), `group` (CometChat.Group), `onSendButtonClick` ((message: CometChat.BaseMessage) => void), `enableRichTextEditor` (boolean), `placeholderText` (string)
**CSS class:** `.cometchat-compact-message-composer`
...
**Expected result:** The composer input uses the "SF Pro" font, button icons turn pink (#f19fa1), and the send button background changes to red (#e5484d). See screenshot above.
...
| Rich text toolbar not showing | `enableRichTextEditor` not set to `true` | Add `enableRichTextEditor={true}` prop |
