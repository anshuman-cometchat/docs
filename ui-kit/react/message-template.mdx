...
- **Usage:** Pass `templates` prop to `CometChatMessageList`
- **Required setup:** `CometChatUIKit.init()` + `CometChatUIKit.login()`
- **Key properties:** `category: string`, `type: string`, `contentView`, `bubbleView`, `headerView`, `footerView`
- **Related:** Used with `CometChatMessageList` via the `templates` prop
</Accordion>
...
Import path: `import { CometChatMessageTemplate } from "@cometchat/chat-uikit-react";`

<Note>
**Before using message templates:** Ensure `CometChatUIKit.init(UIKitSettings)` has completed and the user is logged in via `CometChatUIKit.login("UID")`. See [React.js Integration](/ui-kit/react/react-js-integration).
</Note>
...
- Define custom rendering for different message types (text, image, video, audio, file, custom) in the message list
- Customize individual parts of the message bubble (header, content, footer, bottom, status info) independently
- Create entirely new message templates for custom message types that your application sends and receives
- Override the default message bubble layout with a fully custom `bubbleView` for complete control over rendering
- Add custom action sheet options (such as "Refresh" or "Pin") to the long-press menu on messages
- Control how reply previews, reactions, and delivery receipts are displayed within message bubbles
- Extend the default set of templates returned by `ChatConfigurator.getDataSource().getAllMessageTemplates()` with your own
...
| --- | --- | --- | --- |
| Customize the header area of a message bubble | Template view slot | `headerView` | `templates.headerView = (msg) => <CustomHeader />` |
| Replace the main content of a message bubble | Template view slot | `contentView` | `templates.contentView = (msg) => <CustomContent />` |
| Add a footer below the message bubble | Template view slot | `footerView` | `templates.footerView = (msg) => <CustomFooter />` |
| Add a bottom section to the message bubble | Template view slot | `bottomView` | `templates.bottomView = (msg) => <CustomBottom />` |
| Replace the entire message bubble layout | Template view slot | `bubbleView` | `templates.bubbleView = (msg, alignment) => <CustomBubble />` |
| Customize the status/timestamp area | Template view slot | `statusInfoView` | `templates.statusInfoView = (msg) => <CustomStatus />` |
| Customize the reply preview | Template view slot | `replyView` | `templates.replyView = (msg) => <CustomReply />` |
| Add custom long-press action sheet options | Template options | `options` | `templates.options = (user, msg, group) => [...]` |
| Create a new template for custom message types | New template | `new CometChatMessageTemplate({...})` | See [New Templates](#new-templates) above |


## Pagination, Batching, and Performance

Pagination does not apply to the MessageTemplate component. MessageTemplate defines the rendering blueprint for individual message bubbles and does not fetch or paginate data. For message list pagination, see the [Message List](/ui-kit/react/message-list) component, which accepts templates via the `templates` prop.

## Error Handling and Retries

Error handling does not apply directly to the MessageTemplate component. MessageTemplate is a data structure that defines how message bubbles render — it does not perform network requests or expose `onError`, `hideError`, or `errorView` props. For error handling in the message display context, see the [Message List](/ui-kit/react/message-list) component.
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Custom template not rendering | Template `type` or `category` doesn't match message | Verify template matches the message's `type` and `category` fields |
...
| Default templates missing | Not using `ChatConfigurator.getDataSource().getAllMessageTemplates()` | Start with default templates and modify from there |
| Custom view returning blank | View function returns `null` or `undefined` | Ensure view function returns valid JSX |
...
The MessageTemplate component requires `CometChatUIKit.init()` to be called with a valid `appId` and `region` before rendering. The user must be logged in via `CometChatUIKit.login()`. Templates are used with the `CometChatMessageList` component, which depends on the `@cometchat/chat-uikit-react` package (includes `@cometchat/chat-sdk-javascript` as a peer dependency).
...
MessageTemplate controls how different message types — including media messages — are rendered in the message list. The `contentView` property determines how each message type is displayed. By default, the UI Kit provides built-in templates for image, video, audio, and file message types, each with their own bubble rendering. When creating custom templates, you can use the `contentView` slot to define custom rendering for media messages. See the [Contentview](#contentview) example in Advanced Features for a demonstration of custom content rendering.

## Logging, Tracing, and Observability

To troubleshoot issues with message templates (such as custom views not rendering or templates not matching message types), enable CometChat SDK debug logging. Set the log level to `DEBUG` during `CometChat.init()` to see detailed SDK activity in the browser console. Refer to the CometChat SDK documentation for the full logging configuration API.
...
<CardGroup cols={2}>
  <Card title="Message List" icon="messages" href="/ui-kit/react/message-list">
    Use templates with the message list component
  </Card>
  ...
  <Card title="Message Bubble Styling" icon="paintbrush" href="/ui-kit/react/theme/message-bubble-styling">
    Style individual message bubbles with CSS
  </Card>
</CardGroup>
