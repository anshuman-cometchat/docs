...
- **Minimal JSX:** `<CometChatMessageList user={chatUser} />` or `<CometChatMessageList group={chatGroup} />`
- **Required setup:** `CometChatUIKit.init()` + `CometChatUIKit.login()`
- **Key props:** `user: CometChat.User`, `group: CometChat.Group`, `messagesRequestBuilder: CometChat.MessagesRequestBuilder`, `templates: CometChatMessageTemplate[]`, `hideReceipts: boolean`
- **CSS class:** `.cometchat-message-list`
...
Import path: `import { CometChatMessageList } from "@cometchat/chat-uikit-react";`
...
- Display a scrollable list of sent and received messages in a user or group chat
- Render different message types including text, media, stickers, reactions, and threaded replies
- Show read receipts, date separators, and sticky date headers for message context
- Support real-time message updates via `CometChatMessageEvents` (new messages, edits, deletes)
- Customize message rendering with `CometChatMessageTemplate` for custom message bubbles
- Control message fetching with `MessagesRequestBuilder` filters (search, limit, categories)
- Enable threaded conversations with `onThreadRepliesClick` and `parentMessageId`
- Provide custom header and footer views above and below the message list
...
You can set the css to the MessageList Component Component to customize the styling. The root CSS class for this component is `.cometchat-message-list`.
...
| --- | --- | --- | --- |
| Override behavior on user interaction | Component props | `on<Event>` callbacks | `onThreadRepliesClick={() => handleThread()}` |
| Toggle visibility of UI elements | Component props | `hide<Feature>` boolean props | `hideReceipts={true}` |
| Replace a section of the message list | Component props | `<slot>View` render props | `headerView={<CustomHeader />}` |
| Customize message bubbles | Component props | `templates` prop | `templates={getTemplates()}` |
| Control message fetching | Component props | `messagesRequestBuilder` | `messagesRequestBuilder={builder}` |
| Change colors, fonts, spacing | Global CSS | Target `.cometchat-message-list` class | `.cometchat-message-list { background: #fff; }` |

## Pagination, Batching, and Performance

The MessageList component supports pagination through the `messagesRequestBuilder` prop. Use `CometChat.MessagesRequestBuilder().setLimit(n)` to control how many messages are fetched per page. The component handles infinite scroll automatically — as the user scrolls up, older messages are fetched in batches based on the configured limit. See the [Filters > Messages Request Builder](#1-messages-request-builder) section above for the code example.

The `scrollToBottomOnNewMessages` prop controls whether the list auto-scrolls when new messages arrive. The `goToMessageId` prop allows jumping to a specific message on initial load. The `startFromUnreadMessages` prop loads from the first unread message when available.

## Error Handling and Retries

The MessageList component supports an `onError` callback prop that fires when an error occurs during rendering or data fetching. You can use this to log errors or display custom error UI. See the `onError` callback example in [Basic Usage > Actions](#2-onerror) for the implementation pattern.

The `hideError` prop controls whether the default error view is shown. You can also provide a custom `errorView` prop to render your own error UI. The `onError` prop receives a `CometChat.CometChatException` object containing the error details.
...
| Component renders but shows no messages | User not logged in | Call `CometChatUIKit.login("UID")` after init |
| Messages not updating in real-time | WebSocket connection issue | Check [Connection Status](/sdk/javascript/connection-status) |
...
The MessageList component renders media messages (images, videos, audio, files) inline within the message list. Media rendering is handled automatically by the component's built-in message templates. The component supports text messages, media messages (image, video, audio, file), stickers, reactions, and custom message types via the `templates` prop.

You can control sound notifications for incoming messages using the `disableSoundForMessages` and `customSoundForMessages` props documented in the [Functionality](#functionality) table above.

## Logging, Tracing, and Observability

To troubleshoot issues with the MessageList component (such as messages not loading, real-time updates not appearing, or custom templates not rendering), enable CometChat SDK debug logging. Set the log level to `DEBUG` during `CometChat.init()` to see detailed SDK activity in the browser console. Refer to the CometChat SDK documentation for the full logging configuration API.
