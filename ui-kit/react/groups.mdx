...
// With common props
<CometChatGroups
  onItemClick={(group) => console.log("Selected:", group)}
  hideSearch={false}
...
| --- | --- | --- | --- |
| Handle item click | `CometChatGroups` | `onItemClick` | `onItemClick={(g) => setActive(g)}` |
| Enable multi-select | `CometChatGroups` | `selectionMode` | `selectionMode={SelectionMode.multiple}` |
| Filter groups | `CometChatGroups` | `groupsRequestBuilder` | `new CometChat.GroupsRequestBuilder().setLimit(10)` |
| Hide group type | `CometChatGroups` | `hideGroupType` | `hideGroupType={true}` |
...
| List is empty | Filters too strict | Relax `groupsRequestBuilder` filters |
| Group type icon missing | `hideGroupType` enabled | Set `hideGroupType={false}` |
| Callbacks not firing | Wrong prop name | Use `onItemClick` or `onSelect` |
| Styles not applying | CSS Modules used | Use global CSS with `.cometchat-groups` |

## FAQ

**Can I show only joined groups?**
Yes. Use `GroupsRequestBuilder` filters for membership.

**How do I hide the group type icon?**
Set `hideGroupType={true}`.
