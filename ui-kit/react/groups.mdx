...
| Real-time events | `CometChatGroupEvents.ccGroupCreated`, `CometChatGroupEvents.ccGroupDeleted`, `CometChatGroupEvents.ccGroupMemberAdded`, `CometChatGroupEvents.ccGroupLeft` |
...
Group lifecycle events are available via `CometChatGroupEvents` — subscribable from anywhere in the application.

| Event | When it fires | Payload type |
| --- | --- | --- |
| `CometChatGroupEvents.ccGroupCreated` | A new group is created | `CometChat.Group` |
| `CometChatGroupEvents.ccGroupDeleted` | A group is deleted | `CometChat.Group` |
| `CometChatGroupEvents.ccGroupMemberAdded` | Members are added to a group | `{ userAddedIn: CometChat.Group, usersAdded: CometChat.User[], addedBy: CometChat.User }` |
| `CometChatGroupEvents.ccGroupLeft` | A user leaves a group | `{ leftGroup: CometChat.Group, leftUser: CometChat.User }` |
...
| --- | --- |
| `onGroupMemberJoined` | Adds the group to the list or updates member count |
| `onGroupMemberLeft` | Updates member count or removes group if current user left |
| `onGroupMemberKicked` / `onGroupMemberBanned` | Updates group metadata and member count |
