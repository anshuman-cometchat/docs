...
- **Minimal JSX:** `<CometChatMessageComposer user={chatUser} />` or `<CometChatMessageComposer group={chatGroup} />`
...
Import path: `import { CometChatMessageComposer } from "@cometchat/chat-uikit-react";`
...
- Provide a rich text input for composing and sending text, image, video, and custom messages in user or group conversations
- Support file attachments including images, videos, audio, and documents with configurable visibility for each attachment type
- Enable @mentions for users and group members with customizable request builders for fetching mention suggestions
- Add voice recording capabilities so users can send audio messages directly from the composer
- Customize the send button, attachment options, auxiliary buttons, and header view using custom view slots
- Integrate emoji keyboard, stickers, polls, collaborative documents, and whiteboards as composable add-ons
- Listen for message events (`ccMessageSent`, `ccMessageEdited`, `ccReplyToMessage`) to extend functionality across your application
- Control typing indicator behavior with the `disableTypingEvents` prop for privacy-sensitive use cases
...
<Tabs>
<Tab title="MessageComposerDemo (TypeScript)">
```tsx lines
import React from "react";
import { CometChat } from "@cometchat/chat-sdk-javascript";
import { CometChatMessageComposer } from "@cometchat/chat-uikit-react";
...
<Tab title="MessageComposerDemo (JavaScript)">
```jsx lines
import React from "react";
import { CometChat } from "@cometchat/chat-sdk-javascript";
import { CometChatMessageComposer } from "@cometchat/chat-uikit-react";
...
| --- | --- | --- | --- |
| Override behavior on send or text change | Component props | `on<Event>` callbacks | `onSendButtonClick={(msg) => handleSend(msg)}` |
| Toggle visibility of UI elements | Component props | `hide<Feature>` boolean props | `hideVoiceRecordingButton={true}` |
| Replace a section of the composer | Component props | `<slot>View` render props | `sendButtonView={<CustomSendButton />}` |
| Customize attachment options | Component props | `attachmentOptions` | `attachmentOptions={getAttachments()}` |
| Change colors, fonts, spacing | Global CSS | Target `.cometchat-message-composer` class | `.cometchat-message-composer .cometchat-button__icon { background: #f19fa1; }` |

## Pagination, Batching, and Performance

Pagination does not apply to the MessageComposer component. The MessageComposer provides a text input and attachment interface for composing messages and does not fetch or paginate lists of data. For list-based pagination, see the [Conversations](/ui-kit/react/conversations), [Users](/ui-kit/react/users), or [Message List](/ui-kit/react/message-list) components.

## Error Handling and Retries

The MessageComposer component supports an `onError` callback prop that fires when an error occurs during rendering or message sending. You can use this to log errors or display custom error UI.
...
The MessageComposer component provides built-in support for sending file attachments including images, videos, audio, and documents. The visibility of each attachment type can be controlled individually using the `hideImageAttachmentOption`, `hideVideoAttachmentOption`, `hideAudioAttachmentOption`, and `hideFileAttachmentOption` props documented in the [Functionality](#functionality) table above.

You can also hide the entire attachment button using `hideAttachmentButton={true}`, or provide a fully custom set of attachment options via the `attachmentOptions` prop — see the [AttachmentOptions](#attachmentoptions) example in Advanced Features.

Voice recording is supported natively and can be toggled with `hideVoiceRecordingButton`. Additionally, collaborative features like polls (`hidePollsOption`), collaborative documents (`hideCollaborativeDocumentOption`), and whiteboards (`hideCollaborativeWhiteboardOption`) can be shown or hidden as needed.

## Logging, Tracing, and Observability

To troubleshoot issues with the MessageComposer component (such as messages failing to send or attachment errors), enable CometChat SDK debug logging. Set the log level to `DEBUG` during `CometChat.init()` to see detailed SDK activity in the browser console. The `CometChatMessageEvents` (`ccMessageSent`, `ccMessageEdited`, `ccReplyToMessage`) documented in the [Events](#events) section can also be used for observability by subscribing to track message lifecycle states (inProgress, success, error). Refer to the CometChat SDK documentation for the full logging configuration API.
