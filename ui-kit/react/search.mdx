...
- **Minimal JSX:** `<CometChatSearch />`
- **Required setup:** `CometChatUIKit.init()` + `CometChatUIKit.login()`
- **Key props:** `onConversationClicked` (callback), `onMessageClicked` (callback), `searchFilters` (array), `searchIn` (array), `hideBackButton` (boolean)
- **CSS class:** `.cometchat-search`
- **Events:** none
</Accordion>
...
Import path: `import { CometChatSearch } from "@cometchat/chat-uikit-react";`
...
- Search across conversations and messages in real time to quickly locate specific content
- Filter search results by conversations, messages, photos, audio, or documents using `searchFilters`
- Scope search to specific entities (conversations only, messages only) using the `searchIn` prop
- Navigate directly to a conversation or message from search results via `onConversationClicked` and `onMessageClicked` callbacks
- Customize the search experience with custom views for conversation items, message items, leading views, trailing views, and more
- Control search result pagination with `ConversationsRequestBuilder` and `MessagesRequestBuilder` to set page sizes and apply filters
- Embed the search component in multiple contexts — conversation list, message header, or as a standalone full-screen experience
...
Using CSS you can customize the look and feel of the component in your app like the color, size, shape, and fonts. The root CSS class for this component is `.cometchat-search`.
...
| --- | --- | --- | --- |
| Override behavior on conversation/message click | Component props | `on<Event>` callbacks | `onConversationClicked={(c) => openChat(c)}` |
| Filter search results by type | Component props | `searchFilters` array | `searchFilters={[CometChatSearchFilter.Messages]}` |
| Scope search to specific entities | Component props | `searchIn` array | `searchIn={[CometChatSearchScope.Conversations]}` |
| Toggle visibility of UI elements | Component props | `hide<Feature>` boolean props | `hideBackButton={true}` |
| Replace conversation item views | Component props | `conversation<Slot>View` render props | `conversationItemView={(c) => <CustomItem />}` |
| Replace message item views | Component props | `message<Slot>View` render props | `messageItemView={(m) => <CustomMessage />}` |
| Customize message timestamp format | Component props | `messageSentAtDateTimeFormat` | `messageSentAtDateTimeFormat={new CalendarObject({...})}` |
| Control pagination page size | Component props | `conversationsRequestBuilder`, `messagesRequestBuilder` | `conversationsRequestBuilder={new CometChat.ConversationsRequestBuilder().setLimit(5)}` |
| Change colors, fonts, spacing | Global CSS | Target `.cometchat-search` class | `.cometchat-search { gap: 0; }` |

## Pagination, Batching, and Performance

The CometChatSearch component supports pagination for both conversation and message search results through `ConversationsRequestBuilder` and `MessagesRequestBuilder`. You can control the page size using the `setLimit` method on each builder — for example, `new CometChat.ConversationsRequestBuilder().setLimit(5)` fetches 5 conversations per page, and `new CometChat.MessagesRequestBuilder().setLimit(5)` fetches 5 messages per page. See the code examples in [Basic Usage > Filters](#filters) for the implementation pattern.

The component handles loading states internally and supports a custom `loadingView` prop to display a custom loading indicator while results are being fetched.

## Error Handling and Retries

The CometChatSearch component supports an `onError` callback prop that fires when an error occurs within the component. You can use this to log errors or display custom error UI.
...
The component also supports an `errorView` prop to display a custom error component when an error occurs. See the Functionality table in [Basic Usage > Functionality](#functionality) for the `errorView` prop.
...
| Component renders but shows no results | User not logged in or no matching data | Call `CometChatUIKit.login("UID")` after init. Verify search term matches existing data. |
...
| Filters not applied | `RequestBuilder` not configured correctly | Verify builder methods and parameters. See [ConversationsRequestBuilder](/sdk/javascript/retrieve-conversations) |
...
The CometChatSearch component requires `CometChatUIKit.init()` to be called with a valid `appId` and `region` before rendering. The user must be logged in via `CometChatUIKit.login()`. The component depends on the `@cometchat/chat-uikit-react` package, which includes `@cometchat/chat-sdk-javascript` as a peer dependency.
...
Files and media handling does not apply to the Search component. The Search component displays search results for conversations and messages but does not render or send media directly. For file and media support, see the [Message Composer](/ui-kit/react/message-composer) and [Message List](/ui-kit/react/message-list) components.

## Logging, Tracing, and Observability

To troubleshoot issues with the Search component (such as search results not appearing or callbacks not firing), enable CometChat SDK debug logging. Set the log level to `DEBUG` during `CometChat.init()` to see detailed SDK activity in the browser console. The `onError` callback prop can also be used to capture and log errors for observability. Refer to the CometChat SDK documentation for the full logging configuration API.
...
<CardGroup cols={2}>
  ...
  <Card title="Search Messages Guide" icon="magnifying-glass" href="/ui-kit/react/guide-search-messages">
    Build a message search experience
  </Card>
