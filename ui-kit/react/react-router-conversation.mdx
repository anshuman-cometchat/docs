...
```tsx CometChatNoSSR.tsx lines highlight={14-16, 42}
    ...
    CometChatUIKit.init(UIKitSettings)
      ?.then(() => {
        ...
        CometChatUIKit.getLoggedinUser().then((loggedInUser) => {
          if (!loggedInUser) {
            CometChatUIKit.login("cometchat-uid-1") //TODO: Replace with dynamic UID
              .then((u) => {
...
<CardGroup cols={2}>
  ...
  <Card title="React Router Integration" href="/ui-kit/react/react-router-integration">
    Return to the React Router setup guide.
  </Card>
