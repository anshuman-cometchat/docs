...
| **setUseInlineAudioRecorder**                  | Switches between inline audio recorder (default) and bottom sheet recorder                                                                                                                                                                      | `.setUseInlineAudioRecorder(boolean)`                       |
...
- `setEnableRichTextFormatting(true)` — Master switch that activates formatting. Without this, even manually typed markdown syntax like `**text**` will be sent as plain text.
- `setRichTextFormattingOptionsVisibility(View.VISIBLE)` — Shows the formatting toolbar above the input field.
- `setShowTextSelectionMenuItems(true)` — Adds formatting options (Bold, Italic, Strikethrough, Code) to the text selection context menu.
...
// Show formatting options in text selection menu
