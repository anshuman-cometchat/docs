...
Triggered when the user taps the audio or video call icon on a call log item. Use this to initiate a call or navigate to a call screen.
...
> **What this does:** Registers a callback that fires when the user taps the call icon on a call log entry. The callback receives the view, view holder, position, and `CallLog` object.
...
| `refreshCallLogs` | Forces a refresh of the call log list, re-fetching data from the server | `.refreshCallLogs();` |
