...
fun unblockUser() {
    CometChatUIKit.unblockUsers(listOf(currentUser.uid), object : CometChat.CallbackListener<List<String>>() {
        override fun onSuccess(uids: List<String>) {
            unblockButtonState.postValue(false)
        }
...
binding.blockMenuItem.setOnClickListener {
    CometChatUIKit.blockUsers(listOf(user.uid), object : CometChat.CallbackListener<List<String>>() {
        override fun onSuccess(uids: List<String>) {
            viewModel.refreshUser()
        }
        override fun onError(e: CometChatException?) {
