...
| Back button not visible | Call `setBackIconVisibility(View.VISIBLE)` on the component instance. The back button defaults to `View.GONE`. |
...
| `setOnBackButtonPressed` not firing | Ensure the back button is visible by calling `setBackIconVisibility(View.VISIBLE)`. If the back button is hidden, the listener will not trigger. |
...
| Typing indicator not showing | The typing indicator only appears when `setSubtitleView` is not set (i.e., using the default subtitle). Custom subtitle views override the typing indicator. |
| Menu icon visible but no popup items | Call `setOptions(menuItems)` with a non-empty list. Setting options automatically makes the menu icon visible; setting `null` hides it. |

## FAQ

**Q: Do I need to pass both a User and a Group object?**
**A:** No. Pass either a `User` object using `setUser(user)` or a `Group` object using `setGroup(group)`. The component displays details for whichever object is set. Setting one clears the other.
...
**Q: Does the MessageHeader component emit any global UI events?**
**A:** No. The `MessageHeader` component does not produce any global UI events. Use action callbacks (`setOnBackButtonPressed`, `setOnError`) to respond to user interactions.

**Q: Why does the typing indicator not appear when I set a custom subtitle view?**
**A:** The typing indicator is rendered in the default subtitle area. When you provide a custom `setSubtitleView`, it replaces the entire subtitle layout including the typing indicator. To show typing status in a custom subtitle, listen to the ViewModel's typing LiveData.

**Q: How do I show the back button?**
**A:** Call `setBackIconVisibility(View.VISIBLE)`. The back button defaults to `View.GONE`.
