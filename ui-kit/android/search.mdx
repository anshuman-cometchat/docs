...
<Tab title="Java">
```java
        binding.cometchatSearch.setImageMessageItemView(new MessagesSearchViewHolderListener<MediaMessage>() {
            @Override
            ...
            @Override
            public void bindView(Context context, View createdView, MediaMessage message, RecyclerView.ViewHolder holder, List<BaseMessage> messagesList, int position) {
                // Bind your data to the custom view here
            }
        });
```
...
<Tab title="Kotlin">
```kotlin
        binding.cometchatSearch.setImageMessageItemView(object : MessagesSearchViewHolderListener<MediaMessage>() {
            override fun createView(
                context: Context?,
                listItem: View?
            ): View? {
                return LayoutInflater.from(context).inflate(R.layout.custom_image_message_item_view, null)
            }
