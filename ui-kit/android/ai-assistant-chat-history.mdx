...
| XML Element | `<com.cometchat.chatuikit.aiassistantchathistory.CometChatAIAssistantChatHistory />` |
| Required | `setUser(User)` with role `@agentic` |
| Callbacks | `setOnItemClickListener`, `setOnItemLongClickListener`, `setOnNewChatClickListener`, `setOnCloseClickListener` |
| Style | `setStyle(@StyleRes int)` with `cometChatAIAssistantChatHistory*` attributes |
