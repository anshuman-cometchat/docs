...
A selection spanning several kinds does not produce one mixed message: the composer sends one message per kind, so images still arrive as a grid and documents as a card stack. They share a `batchId` and are grouped as described below.
