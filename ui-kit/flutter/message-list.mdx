...
<Warning>
A **mixed** batch (for example images plus a PDF) is typed `file`, so every item — images included — renders as a file card rather than a grid. Send kinds separately if you want the grid.
</Warning>
