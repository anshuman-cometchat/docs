...
A selection spanning several kinds is split and sent as one message per kind, in the order image → video → audio → file. They share a `batchId`, so the [Message List](/ui-kit/flutter/message-list#multiple-attachments) renders them as one group.
...
For the upload API underneath the tray, see [Upload Files Before Sending](/sdk/flutter/send-message#upload-files-before-sending).
