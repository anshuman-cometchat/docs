...
<CardGroup rows={3} className="mt-6">
  <Card title="Guest chat (Anonymous)" href="#1-guest-chat-no-login">
    - • Let anyone chat without creating an account. 
    - • Perfect for marketing pages, help centers, or demo sites.
  </Card>
  <Card title="Create and Login user on the fly" href="#2-log-in-with-uid-create-user-automatically">
    - • Reuse an ID you already have (email, username) so returning visitors keep the same name and history.
  </Card>
  <Card title="Log in with your backend" href="#3-log-in-with-your-backend-auth-token-login">
    ...
    - • Keeps the user authentication on server.
  </Card>
</CardGroup>

---

## 1. Guest chat (Anonymous)
...
| --- | --- |
| `appID`, `region`, `authKey` | Copy from **[Dashboard](https://app.cometchat.com) → Settings**. Use the **Guest Auth Key** for this flow. |
| `user.name`, `user.avatar`, `user.link` | Optional guest display info—leave blank if you don’t need it. |
| `width`, `height`, `isDocked` | Adjust the widget size or make it dock/undock. |

---

## 2. Create and Login user on the fly
...
| Setting | What to enter / where to find it |
| --- | --- |
| `appID`, `region`, `authKey` | Same values as the guest flow. |
...
| `width`, `height`, `isDocked` | Adjust layout just like the guest flow. |
...
| Setting | What to enter / where to find it |
| --- | --- |
| `appID`, `region` | Copy from **[Dashboard](https://app.cometchat.com) → Settings**. |
| `mode` | Keep as `"authToken"` so the widget expects a server-issued token. |
| `authToken` | Generate on your **server** after login: (1) call the CometChat Auth Token API (or server SDK) with your REST API key, (2) get the token for that user, (3) send it to the page and place it here. |
| Layout options | `mount`, `width`, `height`, `isDocked` work exactly like the other flows. |
