...
Refreshed for the latest [`ai-agent-crew-ai-examples`](https://github.com/) codebase: run the `product_hunt_agent` FastAPI service, wire it to CometChat, and stream SSE updates (with optional confetti actions).
...
- A FastAPI `/stream` endpoint emitting newline-delimited JSON (`text_delta`, `text_done`, `done`).
...
- `POST /stream` — SSE stream (`text_delta`, `text_done`, `done`) ready for CometChat.
...
@tool("getTopProductsThisWeek")  # rolling-week window, clamps days/limit
@tool("getTopProductsByTimeframe")  # "today", "yesterday", "last_week", ISO, ranges
@tool("searchProducts")          # Algolia search (no token needed)
@tool("triggerConfetti")         # returns payload: colors, counts, spread, etc.
```

All tools run server-side; if `PRODUCTHUNT_API_TOKEN` is missing, top queries return empty arrays but still respond cleanly.

***

## Wire it to CometChat

- Dashboard → AI Agents → **Provider: CrewAI** → **Agent ID** (e.g., `product_hunt`) → **Deployment URL** = your public `/stream`.
- Listen for `text_delta`/`text_done` to render streaming text; stop on `done`.
