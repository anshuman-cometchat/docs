...
description: "Connect a LangGraph (TypeScript) agent to CometChat, customize it in UI Kit Builder, and ship the experience as code or a widget."
---

## What you’ll build

- A **LangGraph** agent (TypeScript) that streams messages and tools.  
...
- A LangGraph endpoint that streams responses (wrap your graph with SSE or newline-delimited JSON). The examples live in **ai-agent-lang-graph-examples**.
- Node.js 18+ with `@langchain/langgraph`, `@langchain/openai`, and access to `OPENAI_API_KEY`.
...
Navigate to **AI Agent → BYO Agents** and then **Get Started / Integrate → Choose LangGraph**.
...
> **Streaming tip:** Expose `app.stream` or `app.streamEvents` as SSE/newline JSON and echo `thread_id` + `run_id` on every chunk so CometChat can stitch partial messages and tool events.
...
- Knowledge agent: `/ai-agents/langgraph-knowledge-agent` — retrieval tool + citations.
- Product Hunt agent: `/ai-agents/langgraph-product-hunt-agent` — multiple tools with looped routing.

Use them as blueprints for your own graphs before wrapping them in your preferred server framework.
