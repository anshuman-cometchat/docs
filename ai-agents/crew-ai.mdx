...
- A **CrewAI** agent exposed via a public endpoint (e.g., FastAPI `/stream`) that streams NDJSON.
...
<Steps>
  ...
  <Step title="CrewAI configuration">
    ...
    <ul>
      ...
      <li><b>Deployment URL</b> — the public HTTPS endpoint exposed by your CrewAI service (e.g., <code>/stream</code>).</li>
...
If you need a starter CrewAI endpoint, you can reuse the `/stream` pattern from your CrewAI project:
