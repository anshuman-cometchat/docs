...
Reference context data in your agent instructions using template variables:

```javascript
const agent = new Agent({
  name: 'Shopping Assistant',
  instructions: `
    You are a shopping assistant for user {{sender.uid}}.
    {{#if sender.role}}
    User role: {{sender.role}}
    {{/if}}
    
    Always use the user's current page context to answer questions
    about the specific product they're viewing.
  `,
  model: openai('gpt-4'),
