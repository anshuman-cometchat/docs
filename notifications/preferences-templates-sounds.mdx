...
End users who have enabled Do Not Disturb (DND), muted a conversation, or set up a schedule will still receive notifications when @all is used in a group message, if this setting is enabled. This ensures all group members are notified regardless of their individual notification preferences.
