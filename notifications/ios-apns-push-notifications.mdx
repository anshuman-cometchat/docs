...
- CometChat App ID, Region, Auth Key; Push Notifications extension with **APNs Device provider ID** and **APNs VoIP provider ID**.
...
- From the sample repo, copy into your target:
  ...
  - `NotificationExtension/` (optional, for delivery receipts)
...
- `UNUserNotificationCenterDelegate.didReceive`: route taps to chats using the sample helper (`CometChatPNHelper.handleTapActionOnNotification`).
