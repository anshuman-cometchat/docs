...
These map to the options shown in the Notifications → Preferences screen (see dashboard screenshot):

- **Privacy setting**: Choose default/always-privacy/templates with user override for message previews.
- **Group conversations**: New messages/replies (all / mentions-only / none), reactions (all / own / none), message edit/delete (on/off), member joins/leaves/added/kicked/banned/unbanned/scope change (on/off). Overrides are allowed by default.
- **One-on-one**: New messages/replies (notify / don't notify), reactions (all / own / none), edit/delete (on/off).
- **Mentions**: Bypass DND/mute/schedule for direct mentions and optionally for @all in groups.
- **Call preferences**: Call events default to notify (cannot override).
- **Mute preferences**: Allow users to mute all (DND), mute group, mute one-on-one; includes notification schedule.

The rest of this page shows how to read/update these preferences with the SDK.
