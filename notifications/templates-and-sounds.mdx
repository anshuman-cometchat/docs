...
This check runs before template resolution. A skipped message produces no notification activity at all.
...
For the notification body to resolve correctly, ensure your custom message includes a `data.text` field. Without it, the `{{message.data.text}}` placeholder falls back to the configured fallback value.
</Note>
