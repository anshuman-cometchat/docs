...
class VoipNotificationHandler {
  ...
  async getPermissions() {
    ...
    try { await RNCallKeep.checkPhoneAccountEnabled(); } catch {}
  }
  ...
  async displayNotification({ title, body, data }: { title: string; body: string; data?: any }) {
    if (Platform.OS === "android" && !this.channelId) await this.createNotificationChannel();
    await notifee.displayNotification({
      title,
      body,
      data,
      android: this.channelId ? { channelId: this.channelId, smallIcon: "ic_launcher" } : undefined,
    });
  }
  ...
  async displayCallAndroid() {
    ...
    await RNCallKeep.displayIncomingCall(this.callerId, callerName, callerName, "generic");
  }
  ...
  onAnswerCall = async ({ callUUID }: { callUUID: string }) => {
    ...
    setTimeout(async () => {
      ...
      if (!loggedInUser) {
        ...
        await setPendingAnsweredCall({ sessionId: sessionID, raw: this.msg, storedAt: Date.now() });
...
if (pending && !isPendingStale(pending)) {
  try { await CometChat.acceptCall(pending.sessionId); } catch (err) { console.log(err); }
}
