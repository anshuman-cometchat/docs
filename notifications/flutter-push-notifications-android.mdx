...
The Flutter UI Kit push sample already solves FCM permissions, background handlers, full-screen incoming-call UI, and navigation from terminated state. This guide distills that reference so you can mirror it in your app.
...
- CometChat app credentials (App ID, Region, Auth Key) plus Push Notification extension enabled with an **FCM provider** for Flutter Android.
