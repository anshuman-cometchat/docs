...
CometChat's Enhanced Push Notification payload includes an `unreadMessageCount` field (a string) representing the total unread messages across all conversations for the logged-in user. On iOS the badge can be updated from both the client side and the server side.

### 7.1 Add the `app_badge_plus` dependency
...
### 7.3 Server-side badge via APNs

If you use APNs directly (not FCM on iOS), the CometChat push notification payload includes the badge count in the APNs `aps` dictionary. iOS updates the app icon badge automatically when the push is delivered — no client code is needed for this path.

### 7.4 Clear badge when the app opens

Reset the badge count when the app comes to the foreground:
