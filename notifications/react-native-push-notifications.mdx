...
The React Native push sample already wires FCM (Android), APNs + VoIP (iOS), CallKeep, Notifee banners, and deep links into conversations. This guide distills that reference so you can mirror it in your app without relying on `push-integration.mdx`.
...
- CometChat app credentials (App ID, Region, Auth Key) and Push Notifications extension enabled with an **FCM provider (React Native Android)** and **APNs/VoIP provider** for iOS.
