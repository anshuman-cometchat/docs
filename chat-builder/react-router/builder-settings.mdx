...
The `CometChatSettings` object controls everything the React Router Builder renders—messaging, AI helpers, calls, layout, theming, no‑code widget options, and agent tools. Use this page as a definitive map of every toggle with recommended defaults.
...
Settings for the docked, no-code chat widget.

| Property | Type | Description |
| --- | --- | --- |
| docked | boolean | Display chat in docked mode. |

### 5.1 Styles
| Property | Type | Description |
| --- | --- | --- |
| buttonBackGround | string | Docked button background color (hex). |
| buttonShape | string | Button shape, e.g., `'rounded'`. |
| openIcon | string | Icon URL when chat is closed. |
| closeIcon | string | Icon URL when chat is open. |
| customJs | string | Custom JavaScript to inject. |
| customCss | string | Custom CSS to inject. |
| dockedAlignment | string (optional) | Position of docked button: `'left'` or `'right'`. |
