...
After successful creation of the group, you will receive an instance of [`Group`](/sdk/reference/entities#group) class which contains all the information about the particular group.
...
This method returns an Object which has two keys: `group` & `members` . The group key has the [`Group`](/sdk/reference/entities#group) Object which contains all the information of the group which is created. The members key has the `UID` of the users and the value will either be `success` or an `error` message describing why the operation to add/ban the user failed.
