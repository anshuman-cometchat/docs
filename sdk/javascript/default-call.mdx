...
<Note>
  After the call is accepted, you need to start the call session. See the [Call
  Session](/sdk/javascript/direct-call#start-call-session) guide for details on
  starting and managing the actual call.
</Note>
...
<Warning>
  Always remove call listeners when they're no longer needed (e.g., on component
