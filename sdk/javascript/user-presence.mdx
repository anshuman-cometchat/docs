...
The listener callbacks provide a [`User`](/sdk/reference/entities#user) object with presence fields populated. Access the response data using getter methods:
...
- **Use unique listener IDs** — Use unique, descriptive listener IDs (e.g., `"chat-screen-presence"`) to avoid accidentally overwriting other listeners.
...
- **Presence events stop after navigation** — The listener was removed or the component unmounted. Re-register the listener when the component mounts again.
...
- **`lastActiveAt` is `0` or `null`** — The user has never been online or data is not yet available. Handle this in your UI with a fallback like "Never active".
</Accordion>
</AccordionGroup>
