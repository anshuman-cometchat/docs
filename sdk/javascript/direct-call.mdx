...
<Note>
  The Ringing flow requires calling methods from both the Chat SDK
  (`CometChat.endCall()`) and the Calls SDK (`CometChatCalls.endSession()`) to
  ensure proper call termination and participant notification.
</Note>
...
<Tabs>
  <Tab title="JavaScript / TypeScript">
    ```javascript CometChatCalls.endSession(); ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="TypeScript">```typescript CometChat.clearActiveCall(); ```</Tab>
  <Tab title="JavaScript">```javascript CometChat.clearActiveCall(); ```</Tab>
</Tabs>
