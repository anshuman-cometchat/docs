...
|-------|-------|----------|
| No messages returned | Conflicting filters | Simplify filters and add them one at a time |
| Missing message types | Category not included | Ensure category matches type (e.g., "message" for "text") |
| Pagination not working | New request object | Reuse the same MessagesRequest object for pagination |
| Thread replies included | `hideReplies` not set | Add `.hideReplies(true)` to exclude thread messages |
| Deleted messages showing | Default behavior | Add `.hideDeletedMessages(true)` to filter them out |
</Accordion>
