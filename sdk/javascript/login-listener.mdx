...
<AccordionGroup>
  <Accordion title="Use unique listener IDs">
    Each listener must have a unique ID string. If you register a new listener with the same ID, it will replace the previous one. Use descriptive IDs like `"LOGIN_LISTENER_MAIN"` or `"LOGIN_LISTENER_AUTH_SCREEN"` to avoid accidental overwrites.
  </Accordion>
  ...
  <Accordion title="Register early, remove on cleanup">
    Register login listeners as early as possible in your app lifecycle (e.g., right after `init()`). Remove them only when the component or page that registered them is destroyed.
  </Accordion>
</AccordionGroup>
