...
|-------|-------|----------|
| Not receiving messages | Listener not added | Verify `addMessageListener()` was called with correct ID |
| Duplicate messages | Multiple listeners | Ensure only one listener per ID; remove on unmount |
| Missing messages | App was offline | Use `getLastDeliveredMessageId()` + `fetchNext()` for missed messages |
| Wrong message count | Blocked users included | Use `hideMessagesFromBlockedUsers` parameter |
| History not loading | Wrong method | Use `fetchPrevious()` for history, `fetchNext()` for missed messages |
</Accordion>
