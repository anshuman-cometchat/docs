...
After configuring the builder, call `build()` to get the `GroupsRequest` object, then call `fetchNext()` to retrieve groups. Call `fetchNext()` repeatedly on the same instance to paginate.
...
Returns an object with GUIDs as keys and online member counts as values (e.g., `{ "cometchat-guid-1": 5 }`).
