...
Attach the `Attachment` objects from `onFileUploaded` to one `MediaMessage`. Use `attachments` (not `files` — the bytes are already on storage), and set `type` to the kind those attachments are.
