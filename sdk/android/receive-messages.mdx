...
How do I receive messages when my app is running?

For every activity or fragment you wish to receive messages in, register the `MessageListener` using the `addMessageListener()` method.
...
How do I receive messages that I missed when my app was not running?
...
How do I fetch the messages I've not read?

Using the `MessagesRequest` class described above, you can fetch the unread messages for the logged-in user. To achieve this, set the `unread` variable in the builder to `true` using the `setUnread()` method so that only the unread messages are fetched.
...
How do I fetch the message history for a user or a group conversation?
...
To search messages, use the `setSearchKeyword()` method. This method accepts a string as input. When set, the SDK will fetch messages that match the `searchKeyword`.
...
How do I find out the number of unread messages that I have?
...
To get the unread message count for a particular user (with respect to the logged-in user), use the `getUnreadMessageCountForUser()` method.
...
To get the unread message count for a particular group, use the `getUnreadMessageCountForGroup()` method.
...
To get all the unread message counts, use the `getUnreadMessageCount()` method. This method has two variants:
...
How do I find out the number of unread messages I have for every user?

To fetch the unread message counts for just the users, use the `getUnreadMessageCountForAllUsers()` method.
...
How do I find out the number of unread messages for every group?

To fetch the unread message counts for all groups, use the `getUnreadMessageCountForAllGroups()` method.
