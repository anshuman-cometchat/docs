...
This guide explains how to implement a complete calling workflow with ringing functionality, including incoming/outgoing call UI, call acceptance, rejection, and cancellation.
...
<AccordionGroup>
  <Accordion title="onIncomingCallReceived(call: Call)">
    Invoked when an incoming call is received from another user or group.
    
    **Parameters:**
    - `call`: `Call` object containing caller details, session ID, and call type
    
    **Use case:** Display incoming call UI with caller information, play ringtone, show accept/reject buttons.
  </Accordion>
  
  <Accordion title="onOutgoingCallAccepted(call: Call)">
    Invoked on the caller's device when the receiver accepts the call.
    
    **Parameters:**
    - `call`: `Call` object with updated call status
    
    **Use case:** Dismiss outgoing call UI, generate call token, and start the call session using `startSession()`.
  </Accordion>
  
  <Accordion title="onOutgoingCallRejected(call: Call)">
    Invoked on the caller's device when the receiver rejects the call.
    
    **Parameters:**
    - `call`: `Call` object with rejection status
    
    **Use case:** Dismiss outgoing call UI, show "call rejected" message, update call history.
  </Accordion>
  
  <Accordion title="onIncomingCallCancelled(call: Call)">
    Invoked on the receiver's device when the caller cancels the call before it's answered.
    
    **Parameters:**
    - `call`: `Call` object with cancellation status
    
    **Use case:** Dismiss incoming call UI, stop ringtone, log missed call.
  </Accordion>
  
  <Accordion title="onCallEndedMessageReceived(call: Call)">
    Invoked when a call ends and a call-ended message is received. This is useful for updating UI or call history when the call terminates.
    
    **Parameters:**
    - `call`: `Call` object with final call status and duration
    
    **Use case:** Update call history, show call duration, clean up resources.
  </Accordion>
</AccordionGroup>
...
**User who ends the call** (receives `onCallEndButtonPressed()` callback):
