...
// Initiate with custom timeout (90 seconds)
await CometChat.initiateCall(call, 90);
...
An optional `timeout` parameter (in seconds) controls how long the SDK waits for the receiver to answer before automatically cancelling the call. If omitted or invalid, the default timeout of 45 seconds is used. The value must be a positive integer — decimals are floored, and non-positive or non-numeric values fall back to the default.
...
// With custom timeout (90 seconds)
CometChat.initiateCall(call, 90).then(
  (outgoingCall) => {
    console.log("Call initiated with 90s timeout:", outgoingCall);
  },
  (error) => {
    console.log("Call initiation failed:", error);
  }
...
| `timeout`      | Optional. No-answer timeout in seconds. Must be a positive integer. Default: `45` |
