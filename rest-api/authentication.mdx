...
<Note>
**Management APIs** use a different authentication mechanism — HTTP Basic Auth with your management key and secret, not the `apikey` header. See the [Management APIs overview](/rest-api/management-apis) for details.

```bash
curl -X GET "https://api-us.cometchat.io/v3/apps" \
  -u "YOUR_MANAGEMENT_KEY:YOUR_MANAGEMENT_SECRET"
```
